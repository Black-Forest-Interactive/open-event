...
export class RegistrationStatusComponent {
  ...
  set data(info: RegistrationInfo | undefined) {
    if (info) {
      let totalAmount = info.participants
        .filter((p) => !p.waitingList)
        .reduce((sum: number, p: Participant) => sum + p.size, 0);
  ...
  private updateIndicator() {
    ...
    if (
      this.space.available >= this.maxIndicatorSize() ||
      !this.spaceAvailable
    ) {
      this.indicator = [];
    } else {
      this.indicator = Array.from({ length: this.space.available }, (_, i) =>
        i < this.space.remaining
          ? { key: i, value: "bg-green-500" }
          : { key: i, value: "bg-orange-300" },
      );
    }
  }
}
