...
export class RegistrationTableComponent {
  ...
  registrationResource = resource({
    ...
    loader: (param) => {
      return toPromise(
        this.service.getRegistrationDetails(param.request.registration.id),
      );
    },
  });
  ...
  loading = computed(
    () => this.registrationResource.isLoading() || this.updating(),
  );
  ...
  editParticipant(part: Participant) {
    ...
    let dialogRef = this.dialog.open(
      RegistrationParticipantEditDialogComponent,
      {
        data: {
  ...
  removeParticipant(part: Participant) {
    this.updating.set(true);
    let dialogRef = this.dialog.open(
      RegistrationParticipantRemoveDialogComponent,
      {
        data: {
          registration: this.registration()!!.registration,
          participant: part,
