...
export class EventConfirmComponent {
  ...
  loading = computed(
    () => this.eventResource.isLoading() || this.settingsResource.isLoading(),
  );
  error = computed(
    () => this.eventResource.error() || this.settingsResource.error(),
  );
  ...
  confirmationPossible = computed(() =>
    this.isConfirmationPossible(this.participantId(), this.status()),
  );
  requireValidateCode = computed(
    () => this.settings()?.requireValidateCode ?? true,
  );
  ...
  constructor(
    ...
    this.route.queryParams
      ...
      .subscribe((p) => this.handleQueryParams(p));
    this.route.paramMap
      .pipe(takeUntilDestroyed())
      .subscribe((p) => this.handleParams(p));
    ...
    effect(() => {
      ...
      if (event && settings && participantId)
  ...
  private isConfirmationPossible(
  ...
  private handleConfirmationResponse(
    response: ExternalParticipantConfirmResponse,
  ) {
    let participant = response.participant;
    if (response.status == "FAILED" || !participant) {
      this.translate
        .get("registration.dialog.response.error")
        .subscribe((v) => this.handleError(v));
    } else {
  ...
  private updateAutoCompletion(
    event: PublicEvent,
    settings: EventParticipationSettings,
