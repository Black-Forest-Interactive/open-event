...
export class RegistrationModerationComponent {
  ...
  ngOnInit() {
    this.adminOrManager = this.authService.hasRole(
      Roles.REGISTRATION_MANAGE,
    ...
    if (principal)
      this.adminOrManager =
        principal.roles.find(
          (r) =>
            r === "openevent.registration.manage" ||
            r === "openevent.registration.admin",
        ) != null;
  }
  ...
  private requestEditParticipant(
    ...
    this.reloading = true;
    this.service
      .moderationChangeParticipant(
        this.registration.registration.id,
        participant.id,
        request,
      )
      .subscribe((r) => this.handleParticipateResponse(r));
  }
  ...
  private handleParticipateResponse(response: ParticipateResponse) {
    switch (response.status) {
      case "ACCEPTED":
        this.translation
          .get("registration.message.accepted")
          .subscribe((msg) => this.hotToast.success(msg));
        break;
      case "WAITING_LIST_DECREASE_SIZE":
      case "WAITING_LIST":
        this.translation
          .get("registration.message.waiting")
          .subscribe((msg) => this.hotToast.info(msg));
        break;
      case "DECLINED":
        this.translation
          .get("registration.message.declined")
          .subscribe((msg) => this.hotToast.warning(msg));
        break;
      case "FAILED":
        this.translation
          .get("registration.message.failed")
          .subscribe((msg) => this.hotToast.error(msg));
        break;
    }
    this.reloadDetails();
  }
  ...
  private updateData() {
    if (this.registration) {
      this.accepted = this.registration.participants.filter(
        (p) => !p.waitingList,
      );
      this.waitList = this.registration.participants.filter(
        (p) => p.waitingList,
      );
      this.userParticipant = this.registration.participants.find(
        (p) => p.author.email === this.authService.getPrincipal()?.email,
      );
    }
  }
  ...
  private requestParticipateManual(request: ParticipantAddRequest) {
    ...
    this.reloading = true;
    this.service
      .moderationParticipateManual(this.registration.registration.id, request)
      .subscribe((r) => this.handleParticipateResponse(r));
  }
