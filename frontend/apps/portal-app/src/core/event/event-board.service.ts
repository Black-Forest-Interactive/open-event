...
export class EventBoardService {
  ...
  request: EventSearchRequest = new EventSearchRequest(
    "",
    undefined,
    undefined,
    false,
  ...
  private updateRange(
    ...
    console.log(
      `[${new Date().toISOString()}] update range ${JSON.stringify(this.range.value)}`,
    );
    let startDate = null;
    if (start) {
  ...
  resetFilter() {
    this.request = new EventSearchRequest(
      "",
      undefined,
      undefined,
      false,
  ...
  private reload(page: number, size: number) {
    if (this.reloading())
      return console.log(
        "Ignore reload " + page + ":" + size + " cause reloading ongoing",
      );
  ...
  private handleData(response: EventSearchResponse) {
    ...
    this.hasMoreElements =
      value.content.length != 0 && this.pageIndex != value.totalPages - 1;
