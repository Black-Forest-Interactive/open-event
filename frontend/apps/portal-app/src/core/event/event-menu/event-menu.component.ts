...
export class EventMenuComponent {
  ...
  editMenuItem = new EventMenuItem(
    "edit",
    "event.action.edit",
    this.handleActionEdit.bind(this),
    false,
  );
  copyMenuItem = new EventMenuItem(
    "content_copy",
    "event.action.copy",
    this.handleActionCopy.bind(this),
    false,
  );
  deleteMenuItem = new EventMenuItem(
    "delete",
    "event.action.delete",
    this.handleActionDelete.bind(this),
    false,
  );
  adminMenuItem = new EventMenuItem(
    "admin_panel_settings",
    "event.action.admin",
    this.handleActionAdmin.bind(this),
    false,
  );
  publishMenuItem = new EventMenuItem(
    "publish",
    "event.action.publish",
    this.handleActionPublish.bind(this),
    false,
  );
  menuItems = [
    this.editMenuItem,
  ...
  private handleActionEdit() {
    if (this.event)
      EventNavigationService.navigateToEventEdit(this.router, this.event.id);
  }

  private handleActionCopy() {
    if (this.event)
      EventNavigationService.navigateToEventCopy(this.router, this.event.id);
  }

  private handleActionShowDetails() {
    if (this.event)
      EventNavigationService.navigateToEventDetails(this.router, this.event.id);
  }

  private handleActionAdmin() {
    if (this.event)
      EventNavigationService.navigateToEventAdministration(
        this.router,
        this.event.id,
      );
  }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result && this.event)
        this.service
          ...
          .subscribe((d) =>
            EventNavigationService.navigateToEventShow(this.router),
          );
    });
  }
